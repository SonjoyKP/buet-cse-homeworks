...
void handelEndVertex(point vertex)
{
    ...
    int helper_Vertex_id=helper[preEdge_id];

    if(allPoints[helper_Vertex_id].point_id==mergeV)
    {
        edge drawEdge;
        drawEdge.pa=vertex;
...
void handelMergeVertex(point vertex)
{
    ...
    if(allPoints[helper_prev_Vertex_id].point_id==mergeV)
    {
        edge drawPrevEdge;
...
void makeMonotone()
{
    ...
    itt=BST_edges.find(allEdge[15]);
    edge testitedge=*itt;
    printf("found edge id=%d ax=%.2lf ay=%.2lf bx=%.2lf by=%.2lf\n",testitedge.edge_id,testitedge.pa.x,testitedge.pa.y,testitedge.pb.x,testitedge.pb.y);
    */
}
